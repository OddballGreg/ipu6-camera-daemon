...
impl CameraDaemon {
    ...
    fn start_camera(&mut self) -> Result<()> {
        ...
        // Stop placeholder first - they can't both write to v4l2sink
        self.stop_placeholder();

        // Small delay to let v4l2loopback release
    ...
    fn stop_camera(&mut self) -> Result<()> {
        ...
        // Small delay then restart placeholder
        std::thread::sleep(Duration::from_millis(100));
        self.start_placeholder()?;

        // Clear any inotify events generated by our own operations
    ...
    fn clear_inotify_events(&mut self) {
        ...
        // Just drain events without processing
        let _ = self.inotify.read_events(&mut buffer);
    }

    fn check_inotify_events(&mut self) -> bool {
        let mut buffer = [0; 1024];
    ...
    fn update(&mut self) -> Result<()> {
        ...
        let client_count = count_device_clients(&self.args.device, &our_pids);
        ...
        // Track when we first detect a client
        let has_client =
        ...
        // Only activate camera after client has been present for activation_delay
        let should_activate = if self.camera_active {
            // Already active, stay active if clients exist
            client_count > 0
        ...
        if should_activate {
            ...
            // No clients - start cooldown
            match self.cooldown_start {
                None => {
                    // Ensure camera has been running for minimum time
    ...
    fn run(&mut self) -> Result<()> {
        ...
        while self.running.load(Ordering::Relaxed) {
            ...
            std::thread::sleep(Duration::from_millis(self.args.poll_interval_ms));
        }
